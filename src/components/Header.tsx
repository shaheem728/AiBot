...
import { logout } from "@/app/redux/slices/userDetailSlice";
      ...
      {!user && (
        ...
        >
          <span  className="text-sm  text-white">
            Sign up and get 20% off to your first order. <Link href="/user/signup"><u>Sign Up Now </u></Link>
          </span>
        </div>
      )}
      <nav className="bg-white border-gray-200 dark:bg-gray-900 sticky top-0 z-10">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          <button
            ...
            aria-controls="navbar-user"
            aria-expanded="false"
            className="block lg:hidden text-black dark:text-gray-400 focus:outline-none focus:ring-4 rounded-lg text-sm p-2.5 me-1"
          >
            <svg
              className="w-5 h-5"
              aria-hidden="true"
          ...
          <div className=" hidden lg:block">  
           ...
           <Link href="/cart" className="relative inline-flex items-center p-3 text-sm font-medium text-center text-white">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          <Dropdown label="" renderTrigger={() => <span><svg
                width="22"
                height="22"
                viewBox="0 0 22 22"
                fill="none"
             ...
             {
                !user &&<div className="w-28 text-center"> 
                 <Dropdown.Item onClick={()=>router.push('/user/login')}>Login</Dropdown.Item>
                 <hr/>
                 <Dropdown.Item onClick={()=>router.push('/user/signup')}>SignUp</Dropdown.Item>
                </div>
              }
              {
                user &&<> 
                <div className="hidden md:block">
                  <Dropdown.Item onClick={()=>router.push('/user/profileInfo')}>Profile</Dropdown.Item>
                </div>
                <div className="md:hidden">
                  <Dropdown.Item onClick={()=>router.push('/user/profile/')}>Profile</Dropdown.Item>
                </div>
                <Dropdown.Item onClick={()=>router.push('/user/orders_review')}>My Orders</Dropdown.Item>
                <hr/>
                <Dropdown.Item onClick={()=>{logout();setUser('');router.push('/')}}>Logout</Dropdown.Item>
                {
                  id === 1 &&<>
                <hr/>
